export default {
   download:{
	...
	home:{
        navigatorTop:'APP DOWNLOAD',
		 navigator: [
          ...
          {title:'응용','path':'/application'},
          {title:'기술','path':'/technology'},
        ...
        img:'./static/img/superprodehan.png',
				logo:'슈퍼 프로베',
				description:'슈퍼 프로베 시스템은 식품 검측 스마트 장비의 한가지로 Ifoods chain 스마트 하드웨어 실험실에서 육류식품 검측 상대로 연구제조한 성숙된 애플리케이션 형식이다.'
			}
		 },
    ifood:{
      title:"IFOOD 거래소 시세"
		 ...
		 application: {
		 	title:'IFOODS CHAIN응용 환경',
		 	description:'ifoods Chain을 기반으로 여러거지 DAPP애플리케이션, 스마트장비 식품검측 애플리케이션, 채굴, 분포식 데이터 가치 전달, 보호 를 개발할 것이고 분포식 식품검측 네트워크, 분포식 생태유지와 관리, 분포식 인터넷 상가 등 애플리케이션을 개발할 것이다. ',
      readmore:'더보기>>',
		 	items:[
             {iconurl:'./static/img/icon (4).png',title:'스마트장비 식품검측과 채굴',des:'블록체인 자체는 수정불가성이 있고 체인상 각 블록들은 공동으로 장부정보에 대한 보수를 참여하고 있다. Ifoods chain생태중의 참여자들은 모두 둔감화 데이터 채굴을 공유가 가능하며 IFOOD의 포상을 받을수 있다. '},
             {iconurl:'./static/img/icon (1).png',title:'분포식 데이터 가치 보호와 전달',des:' Ifoods chain은 블록체인기술을 기반으로 개인과 기구의 관련 데이터를 보호한다. 동시 가치전달을 위해 데이터 모델 유료사용과 검측 데이터 매매를 한다.'},
             {iconurl:'./static/img/icon (5).png',title:'분포식 식품 검측 네트워크',des:'블록체인의 공유, 개방, 분포식을 기반으로 다방면 참여하는 분포식 식품검측 네트워크를 실현한다. 식품검측에 참여하는 자는 더이상 센터화된 검측기구뿐만 아니라 슈퍼 프로베 등 스마트장비의 소유자가 될수도 있다.'},
             {iconurl:'./static/img/icon (2).png',title:'분포식 생태 유지 및 관리',des:'분포식 식품안전검측 생태를 유지하기 위해  Ifoods chain는 거래 매칭 기질, 지불 기질, 평가 피드백 기질을 가지고 있는 동시 슈퍼 절점에 따라 투표관리, 전민 참여, 식품안전품질 검측 생태 공동유지를 진행한다.'},
             {iconurl:'./static/img/icon (3).png',title:'분포식 인터넷 상가 네트워크 ',des:' Ifoods chain는 프로베 시스템으로부터 빅데이터, 블록체인 기술, AI에 구축된 분포식 인터넷 상가 DAPP을 획득함으로 분포된 인터넷 상가 식품의 안전, 고품질, 가검증을 확보한다.'},
		 	]
		 },
     painPoint:{
      title:'업계 에로',
      item1:'소비자들이 식품에 관련된 지식이 부족하다.',
      item2:'전통적인 검측방식은 시간이 오래 걸리고 원가가 높고 과정이 복잡하다.',
      item3:'검측시장 공급처에 대한 믿음과 루트가 결핍하다.',
      item4:'편리하고 원가가 저렵한 검측수단이 부족하다.',
    ...
    // 超级探针
     superProbe: {
      title:'슈퍼 프로베 ',
      des:'슈퍼 프로베는 Ifoods chain 스마트 하드웨어 실험실에서 개발한 것이고 육류 품질 검측을 위해 장비이며 슈퍼 프로베는  DAPP와 결합시켜 동시 사용해야 한다. ',
    ...
    // 路线图
     roadMap:{
      ...
      year2018:{
        year:'2018',
        month9:'9월',
        event9:'슈퍼 프로베 스마트 하드웨어 DEMO버전 발표',
        month10:'10월',
        event10:'세계 범위에서 슈퍼 프로베 데모기 테스트 ',
        month11:'11월',
        event11:'Ifoodschain 분포식 SNS 온라인',
      ...
      year2019:{
        year:'2019',
        month1:'1월',
        event1:'슈퍼 프로베 글로벌 발매',
        month3:'3월',
        event3:'슈퍼 프로베 중국지역 홍보계획 발표',
        month5:'5월',
        event5:'슈퍼 프로베 유럽아메리카 홍보계획 발표',
        month7:'7월',
        event7:'DAI 테스트 기능 발표',
        month8:'8월',
        event8:'슈퍼 프로베 대형기구 사인회',
        month10:'10월',
        event10:'슈퍼 프로베 판매량 1만대 도달 (예측)',
        month11:'11월',
        event11:'슈퍼 프로베DAPP 디지털자산포켓 온라인',
        month12:'12월',
        event12:'Ifoods Chain 메인 사이트 온라인',

      },
      year2020:{
        year:'2020',
        month6:'06월',
        event6:'오일류 검측 스마트 하드웨어 발표',
        month12:'12월',
        event12:'슈퍼 프로베 판매량 3만대 도달',
      }
	...
	ecomodel: {
		banner:{
			title:'생태 모델',
			des:'Ifoods chain 생태는 블록체인의 분포식 식품안전 품질 검측에 기반한 생태이다. Ifoods chain에서 검측 전문가, 검측자, 감독부문, 식품업체, 상가, 소비자는 공동으로 생태에 참여한다. '
		},
		ecomodelItems:[
           {url:'static/img/jiancez.png',name:'검측 전문가',des:'Ifoods chain 생태에서 각 식품영역 검측 전문가들은 육류식품, 야채식품, 물, 오일 등에 관한 믿음직한 식품 데이터 분석모델을 제공한다. (Ifoodschain은 SNS 투표 및 스마트 데이터 분석모델으로 전문가가 제공한 식품 데이터 분석모델의 믿음성을 확인하는 동시 코체인 가능여부를 확인한다.)'},
           {url:'static/img/jincez1.png',name:'검측자',des:'Ifoods chain 생태에서 검측자는 식품검측 스마트장비로 식품 데이터 채취를 진행하고 식품 모델 라이브러리를 선택하여 분포식 식품 검측을 진행하는 생태 참여자이다. '},
           {url:'static/img/jianguan.png',name:'감독부문',des:'Ifoods chain 생태에서 감독부문은 식품검측 스마트장비를 이용하여 쾌속하고 즉시로 식품 데이터 분석을 진행, 검측효율과 행정효율을 향상시킨다.'},
           {url:'static/img/qiye.png',name:'식품업체와 상가',des:'Ifoods chain 생태에서 식품업체와 상가는 식품검측 스마트장비를 이용하여 쾌속하고 즉시로 식품 데이터 분석을 진행, 식품 검측 원가를 낮춘다. '},
           {url:'static/img/fenbu.png',name:'분포식 인터넷 상가',des:'Ifoods chain 생태중 SNS투표로 선택한 고품질 식품업체를 분포식 공급상의 참여자로 한다. '},
           {url:'static/img/xiaofei.png',name:'소비자',des:'Ifoods chain 생태중 안전식품 구매자, 검측 데이터 구매자이다. 소비자가 검측자가 될수 있고 검측자도 소비자가 될수 있다. '},
		]
	},
	footer:{
		...
		Ecological:{
			title:' 생태',
			subtitle:['생태 모델','기술','애플리케이션']
		},
		About:{
			title:'회사 관련 ',
			subtitle:['팀','뉴스','노선도','프라이버시 정책']
		},
		Contact:{
			title:'연락처'
		},
		disclaimer:'본 사이트에 제공된 내용은 단 참고용으로 사용할 것이며 본 사이트는 정보의 정확성, 유효성, 즉시성과 완정성을 보장하지 않는다. 본 사이트 및 그 종업원은 어떤 방식으로 전달되거나 전달 실수, 불정확 혹은 실수에 대해서 유저나 그 어떤 인원 상대로 직접이나 간접적인 책임을 지지 않는다. 법률이 허락하는 범위내 본 사이트가 성명하는바, 유저나 어떤 인원이 사이트에 제공된 정보나 어떤 링크 사용이나 미사용으로 인한 그 어떤 직접, 간접, 부가, 부속, 특수, 징벌성이나 징계적인 손해배상에 대해 책임지지 않는다. '
	},
  technology: {
    banner: {
      title: '기술 모델',
      des: 'Ifoods chain은 블록체인을 기반으로 만들어진 안전하고 믿음직하며 공유하는 분포식 식품 안전 품질 검측 네트워크이다. '
    },
    technologyitems: [
      {
        url: 'static/img/icon-j.png',
        name: 'Ifoods chain 퍼블릭체인',
        des: 'Ifoods chain의 생태수요를 기반으로, 우리는 OBFT 공명기질을 기반으로 Ifoods chain 퍼블릭체인을 개발할 것이다:<br>' +
        '1.고빈도 거래를 지원하는 능력 소유, tps 지원은 5k이상 도달, 효율은 6초에서 8초 사이. \n<br>' +
        '2.공동 데이터에 대한 저장능력을 소유, 거래한 데이터 구조에 대해 제정과 확장 진행이 가능. \n<br>' +
        '3.대형문서에 대한 비교적 양호한 저장기능을 소유.\n<br>' +
        '4.튜링 완비된 편집이 용이한 스마트 계약 플랫폼 제공 가능. \n '
      },
      {
        url: 'static/img/icon-j8.png', name: 'OBFT 공통인식 기질\n', des: 'Ifoods chain OBFT 공통인식 기질에 포함되는 내용:\n<br>' +
        '1.대량 절점은 SNS투표방식으로 믿음직한 검증절점을 선택, 생태수요에 따라 수시로 검증절점 증가나 삭제 가능.\n<br>' +
        '2.Ifoods chain  라운드마다 블록산출 주기에서 , 하나의 무작위 산법을 통해 검증 절점중의 검증 절점을 선택하여 정보 데티어블록을 기록.\n<br>' +
        '3.OBFT 공통인식 기질은 자원소모를 통과할 필요없이 고빈도 거래 지원이 가능. OBFT 공통인식을 기반으로 Ifoods chain 생태중 tps는 5000이상에 도달 가능.\n '
      },
      {
        url: 'static/img/icon-j7.png',
        name: 'USP 저층 시스템\n',
        des: 'Ifoods chain 저층 구조중, 우리는 일종 다른 유저의  USP(universal service platform) 시스템에 서비스할수 있는 디자인을 진행, Ifoods chain의 핵심구조 기능 및 각 유형 서비스를 봉쇄시켜 Ifoods chain 블록체인 시스템의 미들웨어로 한다. USP시스템은 프론트 애플리케이션에게 개방한 api포트과 sdk를 미리 보류시켜 부동한 유저에게 사용 제공한다. \n<br>' +
        '(1)Ifoods chain 시스템 연결을 간소화와 통일;\n<br>' +
        '(2)유저, 개발자와 기타 업자들이 장소, 시간 불문하고 각종 장비를 사용하여 진입방문과 Ifoods chain 의 서비스를 접수할수 있게 한다. \n'
      },
      {
        url: 'static/img/icon-j2.png',
        name: 'DAI',
        des: 'Ifoods chain의 응용환경에는 거래 매칭, 모델 선택, 데이터 분석 등 관련 기술들이 포함, 이런 기술의 사용에는 모델의 훈련이 포함되어 있고 모두 인공지능을 기반으로한 심도있는 학습산법이고 대량 산력을 소모하기에 우리는 분포식 인공지능을 인입했다(Distributed Artificial Intelligence).\n '
      },
      {
        url: 'static/img/icon-j3.png',
        name: '슈퍼 프로베 시스템\n',
        des: '슈퍼 프로베는 Ifoods chain 스마트장비는 일종 성숙된 애플리케이션 방식이고 육류 검측에 사용한다. 슈퍼 프로베 시스템은 Ifoods chain이 소비자, 상가, 식품업체 상대로 디자인한 MEMS 식품검측  DAPP 블록체인 애플리케이션 시스템이다. 슈퍼 프로베는 휴대하기 편리하고 쾌속 즉시적인 식품에 관련된 데이터 검측이 가능하는 동시 표준 데이터와 대비시켜 소비자의 선택에 도움을 준다. \n'
      },
      {
        url: 'static/img/cunzheng.png',
        name: '블록체인 증거저장\n',
        des: '블록체인 증거저장은 Ifoods chain 블록체인 플랫폼의 중요한 기술이다. 생태구축중, 데이터의 수정불가는 그중 관건이다. 블록체인 증거저장은 식품 안전 품질 검측 정보가 블록체인상의 기록을 보장한다. 이러한 블록체인에 저장된 정보는 최종 식품안전 관련 유효 증거 체인이 된다. 전체 증거체인은 여러 코스, 여러 절점의 참여가 있어 일정한 정도에서 제품정보의 믿음성을 확보한다.\n'
      },
  ...
  superprobe: {
    banner: {
      title: '슈퍼 프로베 \n',
      des: '슈퍼 프로베는 Ifoods chain 스마트 하드웨어 실험실에서 개발한 것이고 육류 품질 검측을 위해 장비이며 슈퍼 프로베는  DAPP와 결합시켜 동시 사용해야 한다. \n',
      url:'static/img/urlko.jpg'

    },
    system: {
      des: '슈퍼 프로베 시스템은 식품 검측 스마트 장비중 (식품검측 스마트장비는 믿음직한 식품 데이터 모델을 기반으로 검측 식품 상대로 데이터 수집을 하는 동시 식품 검측, 위치 등 둔감 데이터를 업로드하는 동시 "채굴"이 가능한 분포식 장비이다.)하나이고 Ifoods chain 스마트 하드웨어 실험실에서 육류 식품 검측 상대로 개발한 성숙된 애플리케이션 형식이다. \n<br>' +
      '<br>' +
      '슈퍼 프로베는 Ifoods chain 스마트 하드웨어 실험실에서 개발한 것이고 육류 품질 검측을 위해 장비이다. \n<br>' +
      '<br>' +
      '슈퍼 프로베는  DAPP와 결합시켜 동시 사용해야 한다. \n<br>' +
      '<br>' +
      '특점: 즉시성, 쾌속성, 휴대편리성\n'
    },
    condition: {
    ...
    conditionitems: [
      ...
      {des: 'DAPP 통한 주변 상가에 대한 조회 \n', url: 'static/img/super3.jpg'},
      {des: '감독관리기구에서 DAPP를 통한 구역내 상가에 대한 분석 \n', url: 'static/img/super4.jpg'},
    ],
    efficient: {
      title: '성능 표준\n'
    },
    efficientitems: [
      {url: 'static/img/time.png', des: '검측시간<120s'},
      {url: 'static/img/length.png', des: '길이<20cm'},
      {url: 'static/img/width.png', des: '너비<10CM'},
      {url: 'static/img/thick.png', des: '두께<5CM'},
      {url: 'static/img/weight.png', des: ' 무게<0.5KG'}
    ]
  },
  application:{
    banner:{
      title:'응용 환경',
      des:'ifoods Chain을 기반으로 여러거지 DAPP애플리케이션, 스마트장비 식품검측 애플리케이션, 채굴, 분포식 데이터 가치 전달, 보호 를 개발할 것이고 분포식 식품검측 네트워크, 분포식 생태유지와 관리, 분포식 인터넷 상가 등 애플리케이션을 개발할 것이다. '
    },
    item1:{
      title:'스마트장비 식품검측과 채굴 (포상)',
      subtitle:'식품검측 스마트장비는 믿음직한 식품 데이터 모델을 기반으로 검측상대 식품에 대해 데이터 수집을 진행한다. 식품검측, 위치 등 둔감데이터를 업로드하는 동시 분포식 장비에 대한  "채굴" 을 진행한다. ',
      des:{
        con1:'식품검측 스마트장비에는 육류 프로베, 액체 프로베, 테스트 지, 오일류 측정기, 잔류농약 검측기, 불량 첨가제 검측기, 유/무기물 감별기 등이 포함되 있다. 스마트장비는 채집된 데이터를 블루투스를 통해 핸드폰 DAPP에 전송하고 핸드폰 DAPP에 채집된 데이터를 현상시켜 유저에게 보여 준다. 유저는 확인후 핸드폰 APP의 포켓 모듈에 사인을 함으로 동시에 데이터를 Ifoods chain 블록체인 시스템에 업로드시켜 영구보존이 가능하다. 동시 스마트장비가  Ifoods chain 후단 검측시스템을 촉발시켜 업로드한 데이터를 입력하여 식품 전문가가 제공한 도구에 결합시켜 계산과 판단을 한다. 그 다음 측정 결과와 결론 피드백을 유저의 핸드폰 APP에 전송시킬수 있고 유저는 동시에 선택으로 검측 결론을 블록체인에 저장할수 있다. 이 결론 데이터는 공개 가능하지만 수정불가한 검사결론으로 하여 가치있는 데이터로 될수 있다.유저는 블록체인 브라우저를 통해 검측한 데이터와 결론을 조회할수 있다.',
        con2title:'채굴 (포상)',
        con2:'참여자는 슈퍼 프로베 등 스마트 하드웨어를 통해 프라이버시를 보호하는 전제하에서 소비정보, 식품 검측정보, 위치정비 등 둔감정보를 공유하며 스마트 계약을 통해 IFOOD 포상을 받는다.'
      }
    },
    item2:{
      title:'분포식 데이터 가치 보호와 전달',
      subtitle:'Ifoods chain는 블록체인 기술을 기반으로 개인과 기구에 관련된 데이터를 보호하는 동시 가치 전달을 위해 데이터 모델 유료사용과 검측 데이터 매매가 이루어 질것이다. ',
      des:{
        con1title:'데이터 모델 사용과 보호 ',
        con1:'전통적인 센터화 검측방식은 유효적으로 전문가 모델 데이터를 보호할수 없다. Ifoods chain은 DAI기술을 통해 전문가 데이어 모델의 안전성을 보장할수 있다. 전문가 기구에서 업로드한 표준 데이터에 대해 Ifoods chain은 합리적인 분류 및 스마트 매칭을 진행할 것이다. 소비자가 데이터 검측 수요가 있을시, DAPP 현상을 통해 소비자가 선택후 스마트 계약을 통해 IFOODS를 데이터 전문가에게 지불을 할수 있으며 다음으로 데이터 검측 진행이 가능하다.',
        con2title:'검측 데이어 매매 ',
        con2:'소비자는 본인이 검측한 데이터의 유일한 소유자이고 데이터를 타인에게 제공사용이 가능하며 IFOOD 보상을 얻을수 있다. ',
        con3title:'둔간 데이터 채굴 공유',
        con3:'Ifoods chain 생태는 유효적으로 모든 참여자의 데이터를 보호하고 그에는 식품 데이터, 소비 데이터, 검측 데이터, 위치 데이터들이 포함된다. 참여자는 본인의 둔감 데이터 공유로 포상을 얻을수 있다. '
      }
    },
    item3:{
      title:'분포식 식품 검측 네트워크',
      des:'블록체인 공유, 개방, 분포식의 공명으로블록체인의 공유, 개방, 분포식을 기반으로 다방면 참여하는 분포식 식품검측 네트워크를 실현한다. 식품검측에 참여하는 자는 더이상 센터화된 검측기구뿐만 아니라 수천수만의 슈퍼 프로베 등 스마트 장비의 소유자가 있다. 그안에는 식품 검측 전문가, 감독기관, 식품업체, 학교식당 등이 포함된다. 공동으로 윈윈하는 가지속 발전적인 분포식 식품검측 네트워크를 구축할 것이다. ',
      subitems:[
        {'iconurl':'static/img/Certified.png',title:'식품 검측 전문가 (기구)',des:'돼지고기 검측 전문가, 세균 검측 전문가, 야채 검측 전문가 등의 연구결과, 특허를 데이터화 한후, Ifoods chain에 업로드시켜 전체 Ifoods chain 퍼블릭체인 생태를 완벽화한다.야채에서 육류까지, 유기물에서 무기물까지, 잔여녹약에서 중금속오렴까지, 식품검측의 분포식 네트워크는 점차 전식품업계 검측영역을 커버하는 것을 실현할 것이다. '},
        {'iconurl':'static/img/agency.png',title:'감독기구',des:'분포식 식품안전 품질 검측 네트워크, 감독부문은 응모 혹은 믿음직한 데이터 구매를 통해 분석을 진행하여 빠르게 구역 식품 품질 데이터를 획득할수 있다. 스마트 하드웨어를 사용하여 식품 안전 감독을 할수도 있고 현장에서 스마트 하드웨어로 감독관리 대기하고 있는 상가의 식품 데이터를 채집할수 있다. 2분내로 검측결과 피드백을 받을수 있어 감독관리 효율 및 감독관리 신뢰도를 향상시킬수 있다. '},
        {'iconurl':'static/img/food.png',title:'식품업체',des:'관련 식품구매업체는 스마트 하드웨어를 사용하여 대량구입한 식품에 대해 검측을 진행할수 있고 Ifoods chain를 통해 쾌속한 검측이 가능하며 단시간내에 결과를 얻을수 있어 추첨 검사 분포도를 증가시키지만 업체 사용원가를 낮출수 있다. '},
        {'iconurl':'static/img/restaurant.png',title:'학교, 식당',des:'학교와 식당등 공공기구에서는 스마트 장비를 사용해 식재료 구매시 식품안전 및 품질검측을 진행하여 식재료의 안전을 확보할수 있는 동시 구매 원가를 낮출수 있다.'}
      ]
    },
    item4:{
      title:'분포식 생태 유지 관리 ',
      subtitle:'분포식 식품 안전 품질 검측 생태를 유지하기 위해 Ifoods chain에는 거래 매칭 기질, 지불 기질, 평가 피드백 기질을 가지고 있다. 동시 슈퍼 절점에 따라 투표관리를 하여 전민참여시켜 공동으로 식품안전 품질 검측 생태를 유지한다. ',
      des:`
                            <p>  <strong>거래 매칭과 지불</strong> <br>
                                Ifoods chain은 표준 데이터에 대해 분류를 하고 검측정보 데이터와 상응 모델의 매칭을 보증한다. 참여자는 DAPP 자유선택 모델에 따라, 동시 스마트 계약을 통해 지불을 진행한다. 거래의 안전과 조회성을 보장하는 동시 제3자의 원가 소모를 줄여준다. 
                            </p>
                            <p> <strong>평가 피드백</strong> <br>
                             모든 표준 데이터를 사용하는 참여자는 모두 사용후 결과에 대해 피드백 평가가 가능하며 블록체인은 평가의 불가변경을 보장할 것이다. 동시 누적된 평가 결과에 대해 스마트 검측을 진행한다. 
                            </p>
                            <p> <strong>SNS 투표</strong> <br>
                            Ifoods chain은 SNS 투표방식을 사용하여 결책을 진행하며 슈퍼 절점에 따라 투표를 진행한다: <br>
                            (1)SNS 투표 데이터 코체인 (전문가, 기구 등 표준 데이터)<br>
                            (2)SNS 투표가 업로드된 악의 데이터에 대한 악의 계정 처리 <br>
                            (3)SNS 투표로 업체 분포식 인터넷 상가 선택

                            </p>`
    },
    item5: {
      title:'분포식 인터넷 상가 네트워크 ',
      des:'Ifoods chain은 프로베 시스템이 획득한 빅데이터, 블록체인 기술, AI에 분포식 인터넷 상가 DAPP 구축을 기반으로 분포된 인터넷 상가 식품의 안전, 고품질, 가검증성을 확보한다. 인터넷 상가 DAPP 입주에는 기타 절점이 그 블록체인에 따른 데이터 투표로 결정이 되고 플랫폼 입주의 공평, 공정을 확보함으로 공동으로 분포식 안전 고품질 식품 판매 생태를 구축한다. 분포식 인터넷 상가 네트워크는 SNS 투표방식을 통해 고품질 식품 공급상을 선택할 것이고 분포식 인터넷 상가 네트워크상의 상가들은 식품 안전 검측 데이터 기록을 Ifoods chain에 업로드할 것이다. 유저는 식품정보를 기반으로 선택구매할 것이고 실기간으로 평가정보를 피드백할 것이다. 분포식 인터넷 상가 DAPP는 소비자의 클릭, 관람, 구매 등 행위 데이터와 거래 데이터를 보호할 것이다. 분포식 인터넷 상가에서 거래는 IFOOD를 사용하여 결제를 진행할 것이고 제3자 원가의 소모를 감소할 것이며 거래의 안전성과 가조회성을 보장할 것이다. '
    }
  ...
  team: {
    ...
    core:{
      title:'운영팀 \n',
      coreitems:[
        {name:'리우왼(刘源）',img:'static/img/liuyuan.png',position:"CTO",des:"블록체인업계의 신앙자 및 조기 전도자, 블록체인 업계의  P2P네트워크, 공식산법, 비대칭 함호 설정, 블록체인 데이터 구조, 스마트 계약 등 기술의 실천원리에 능통하고 BTC，ETH，NEO，EOS 등 대표적인 퍼블릭에 전면적인 역구를 했으며 퍼블릭 체인, 컨소시엄 블록체인, 프라이빗 블록체인의 응용환경에 심도있는 이해를 가지고 있으며 여러 업계 여러 애플리케이션 상대로 저층 블록체인화한 솔루션 설계 및 실행을 진행한 경험이 있다. 예: NEO을 기반으로한 trinity，ETH을 기반으로한 차이베이 시스템, 전력업계 상대로 진행한 블록체인화 솔루션 등. 11년의 인터넷 기초 시설 제품 연구개발을 가지고 있고 과거 차이나 텔레콤, 노키아, 화웨이 등에 임직한 경험이 있으며 개발한 디지털 통신 핵심 교환 제품, 무선 통신 스테이션과 컨트롤 제품, kvm을 기반으로한 가상화 클라우드 컴퓨팅 제품은 선후로 100GE 핵심 교환 시대, 이동 인터넷 시대와 클라우드 컴퓨팅 시대를 개시했다. 블록체인업계의 발전에 따라 우리는 기필코 혁명적인 가치 전송 신시대를 맞을거라고 굳게 믿고 있다. \n"},
        // {name:"순즈워이(孙志伟）",inicon:"static/img/in.png",inUrl:"https://www.linkedin.com/in/sun-zhiwei-25a93137/",img:"static/img/sunzhiwei.png",position:"COO",des:"북경이공대학 MBA; 2015년부터 블록체인을 접촉, 여러 블록체인 프로젝트를 위해 운영 컨설팅 방안 디자인을 제공, 그 안에는 프로젝트 팀 구축, 매체 홍보, SNS 구축 운영, 화폐 가치 관리 등을 포함; 선후로 중국 베이처 그룹, 북경 자동차 그룹, 유명 관리 컨설팅 기구와 투자회사에 임직; A주식 여러 상장회사를 위해 전략 계획, 투자 융자 방안 및 상업 형식 디자인 제공. \n"},
        // {name:"렁링(冷琳",position:"CMO",img:"static/img/lenglin.jpg",des:" 7년간 금융업계 마케팅 홍보 서비스 경험을 가지고 있고 과거 업계내 유명 인터넷 금융, 과기회사를 위해 서비스를 제공, 뉴미디어 환경하의 마케팅방법에 익숙; 블록체인, 재경매체 및 SNS과 긴밀한 소통을 유지.\n"},
        {name:"펑리솽(冯丽爽)",img:"static/img/fenglishuang.png",position:"수석 하드웨어 과학가",des:"북경항공항천대학 연구원, 박사생 지도원, 워이나 테스트 컨트롤 센터 부주임; 국가 \"863\"선진 단체와 개인 표창 받음; MEMS 센서 연구에 전목; 성부급 기술 발명상 1항 획득, 기술 진보상 2항 획득, 중국 발명 특허상 18항 획득; 국내외 발간물과 회의상 100여편 학술 논문을 발표, 그중 20여편이 SCI에 수록됨. \n"}
      ],
    },
    adviser:{
      title:'컨설팅팀 \n',
      adviseritems:[
        {name:"리앤보(李彦博）",inicon:"static/img/in.png",inUrl:"https://www.linkedin.com/in/yanbo-li-b478585/",img:"static/img/liyanbo.png",position:"블록체인 기술 고문",des:"블록체인 기술 전문가, Onchain 연합 창시자; NKN 창시자; Linux Kernel 인터넷층 핵심 코드 공헌자; 스탠퍼드대학 암호과에서 학습, 미국 퀄컴에서 다년간 근무 경험, 분포식 시스템 구축 디자인, 인터넷 협의 실천에 능함; 블록체인 플랫폼 개척 DNA 핵심 개발과 디자이너.\n"},
        {name:"David（리이링/李一灵）",img:"static/img/david.png",position:" 블록체인 운영 고문",des:"Trinity 창시자; FourierPR 연합 창시자; 중국 첨단 암호 경제 프로젝트  PR 및 컨설팅 업체, Fourier의 고객; coinmarketcap 앞순위 100위 리스트 통제, FBG와 합작; 레이스과기 창시자; 미디어 사이트 inwecrypto.com，다자산 포켓 InWeWallet 창시자.\n"},
        {name:"이펑핑(易锋平） ",img:"static/img/yifengping.png",position:"블록체인 정부사무 고문 ",des:"Trinity 연합 창시자; 블록체인 업계 정부사무 전문가, 풍부한 정부종업 배경과 베테랑급 블록체인 프로젝트 루트 확장 경험을 소지; 2015년말 이타이팡 애호가 SNS 진입, 계속 블록체인 애플리케이션과 기술의 사회 및 정부영력에서의 확장에 책임; 전 상해분포정보과기회사 정부사무총감과 퉁지금융과기 체인블록 연구원 부원장; 공신부 블록체인 참고 구조와 정부 관련 블록체인 정책에 대한 편집 작업에 참여; 귀양시 정부에서 하는 국내 최초 성신농민 프로젝트의 현지 합작과 착지 작업을 주도. \n"},
        {name:"차이워이중(柴巍中",img:"static/img/chaiweizhong.png",position:" 식품 안전 고문",des:"영양학 박사(후), 북경대학 공공위생학원 교수, 연구생 지도원, 주로 식품위생학 과학연구와 교학에 종사; 저작 참여 출판한 작품으로는 <유럽 식품 안전 체계>, <중국 영양 산업 발전 보고서>, <공중영양 및 사회경제 발전> 등 있음; 국제 전문 회의와 발간물에서 10편 전문 문장을 발표. \n"},
        {name:"중워이커(仲维科）",img:"static/img/zhongweike.png",position:"식품 검측 기술 고문",des:"중국검사검역과학연구원 남방검측센터 주임, 연구원; 2000년 중국과학원 생태환경연구센터 종업; 장기간 식품과 환경 샘플중 화학오염물 잔여 검측과 과학연구 공장에 종사; 국가자연과학펀트 3항, 과학기술부 과제 3항 주도. 국가표준과 업계표준 5항 제정; 국내외 핵심 발간물에서 40여편 논문을 발표. 식품중 다이옥신, 농산가축 약물 잔여 분석에 능함. \n"}
      ],
    },
    technical:{
      title:'기술팀\n',
      technicalitems:[
        {name:"청성(曾晟）",inicon:"static/img/in.png",inUrl:"https://www.linkedin.com/in/%E6%99%9F-%E6%9B%BE-923275167/",img:"static/img/zengsheng.jpg",position:"시스템 구축사",des:"시스템 구축사, 블록체인 구축사, 베테랑급 풀스택 엔지니어, 전 263 동기하드제품 기술 담당자, 고급 C++/java/go/js 개발 엔지니어, 분포식 시스템 전문가, 분포식 구축 시스템 디자인과 개발에 능함.\n"},
        {name:"리정훠이(李政辉）",inicon:"static/img/in.png",inUrl:"https://www.linkedin.com/in/%E6%94%BF%E8%BE%89-%E6%9D%8E-943275167/",img:"static/img/lizhenghui.jpg",position:"베테랑급 백엔드 개발 엔지니어 ",des:"베테랑급 백엔드 개발 엔지니어, 백엔드 기술 담당자, 백엔드 메인 프로그래머, java/go에 익숙, 통용 인터넷 백엔드 시스템 구축과 분포식 문서 시스템 디자인과 개발에 능함, 백엔드 시스템 개발 경험이 풍부함.\n"},
        {name:"리쟝항(李江航）",inicon:"static/img/in.png",inUrl:"http://www.linkedin.com/in/%E6%B1%9F%E8%88%AA-%E6%9D%8E-0b5277167/",img:"static/img/lijianghang.jpg",position:"베테랑급 프론트 개발 엔지니어",des:"베테랑급 프론트 개발 엔지니어, 프론트 기술 담당자, h5 표준에 익숙, jquery，angular，vue，bootstrap 등 틀에 익숙, javascript에 익숙, ES6 표준, web단과 각종 단 장비 시스템과 app개발에 능함, 개발과 제품 디자인 경험이 풍부함.\n"}
